    ...
    if (self = [super init]) {
        for (NSString *key in dictionary.allKeys) {
            ...
            if ([key isEqualToString:@"testModel"]) {
                JXTestModel *testModel = [[JXTestModel alloc]
                                          initWithDictionary:value];
                value = testModel;
...
#pragma mark - JXEmptyPropertyProperty
...
+ (void)test {
    ...
    NSDictionary *dict = @{@"name"  : @"河流之主",
                           @"title" : @"http://www.jianshu.com/users/370e705f738e/latest_articles",
                           @"count" : @(11),
                           ...
                           @"testModel" : @{@"name"  : @"河流之主",
                                            ...
                                            @"parameters" : @{@"key1" : @"value1", @"key2": @{@"key11" : @"value11", @"key12" : @[@"三层", @"字典->字典->数组"]}},
                                            @"classVersion" : @(1.1)}};
    JXTestModel *model = [[JXTestModel alloc] initWithDictionary:dict];
